...
class EighthAffiliationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.baseline = read(MASTER / 'affiliation_batch8_baseline.json')
        cls.profiles = read(MASTER / 'affiliations_1546.json')['officers']
        cls.report = read(MASTER / 'affiliation_batch8_research.json')
    ...
    def test_exactly_100_new_placements_and_1498_unchanged(self):
        ...
        self.assertEqual(selected | set(read(MASTER / 'affiliation_batch9_baseline.json')['selected_ids']) | set(read(MASTER / 'affiliation_batch10_baseline.json')['selected_ids']) | set(read(MASTER / 'affiliation_batch11_baseline.json')['selected_ids']), {q for q, a in self.profiles.items() if a != self.baseline['officers'][q]})
        for q in selected:
    ...
    def test_100_future_reservations_never_become_start_assignments(self):
        ...
        self.assertTrue(self.rows['石田三成']['after']['future_placement_reserved'])
    ...
    def test_only_registered_known_fathers_supply_minor_houses(self):
        ...
        self.assertEqual(inherited, 67)
        for n in ['福島正則', '糟屋武則', '脇坂安治', '鈴木元信']:
    ...
    def test_same_batch_parent_chains_and_namesakes(self):
        for child, father in [('細川忠利', '細川忠興'), ('長岡休無', '細川忠興'), ('稲葉一通', '稲葉典通'), ('茂庭良元', '鬼庭綱元'), ('松平直基', '結城秀康')]:
        ...
        self.assertEqual(self.rows['蘆名盛隆']['after']['father_assignment_check']['father_id'], 'Q10880505')
        self.assertEqual(self.rows['島津光久']['after']['father_assignment_check']['father_name'], '島津忠恒')
        self.assertEqual(self.rows['結城秀康']['after']['father_assignment_check']['father_name'], '徳川家康')
        self.assertEqual(self.rows['石川忠総']['after']['father_assignment_check']['father_name'], '大久保忠隣')

    def test_unknown_fathers_and_later_geography_remain_provisional(self):
        for name in ['立花宗茂', '立花直次', '真田信繁', '真田幸昌', '豊臣秀頼']:
            self.assertIsNone(self.rows[name]['after']['house_id'])
        self.assertIn('基肄郡', self.rows['筑紫広門 (上野介)']['after']['historical_review_note'])
        self.assertIn('諸説', self.rows['鈴木元信']['after']['historical_review_note'])
        for row in self.rows.values():
    ...
    def test_scores_family_relationships_and_baseline_migration(self):
        ...
        migration = read(MASTER / 'affiliation_batch8_migration.json')
        self.assertEqual(migration['previous_affiliations_sha256'], self.baseline['lineages_baseline']['affiliations_sha256'])
        self.assertEqual(migration['affiliations_sha256'], read(MASTER / 'affiliation_batch9_baseline.json')['lineages_baseline']['affiliations_sha256'])
