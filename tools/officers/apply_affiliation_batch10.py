"""Tenth 100-person overlay, including explicitly authorized future reservations."""
import collections
import copy
import hashlib
import html
...
def apply(data, houses, districts, by_name):
    baseline = read(MASTER / 'affiliation_batch10_baseline.json')
    selected = read(HERE / 'affiliation_batch10_selection.json')
    ...
    assert profiles == baseline['officers'], 'The preceding scenario changed; review before regenerating batch 10'
    before = copy.deepcopy(profiles)
    decisions = {}
    for line in (HERE / 'affiliation_batch10_decisions.txt').read_text(encoding='utf-8').splitlines():
    ...
    for s in selected:
        ...
        assert a['availability'] != 'deceased'
    ...
    for s in selected:
        ...
        if not source_path.exists():
            source_path = ROOT / 'data/sources/officers/affiliation_batch10_summaries' / (qid + '.json')
        ...
        supplements = {
            '水越勝重': ('富山市・富山城址の変遷', 'https://www.city.toyama.toyama.jp/etc/muse/sassi/jyousi-r/jyousi-02r.html', '神保長職家臣の水越勝重が縄張を行ったという伝承'),
            '難波田憲重': ('富士見市・難波田城公園の施設概要', 'https://www.city.fujimi.saitama.jp/madoguchi_shisetsu/02shisetsu/shiryoukan/nanbatajo/sisetu-gaiyou.html', '難波田氏の富士見市の本拠と城跡の説明。没年の証明には使用しない'),
            '中野一安': ('曹洞宗秋月院・寺史と開基中野重吉', 'https://shuugetuin.or.jp/history/', '一安・重吉・又兵衛の表記、織田信秀の家臣として小豆坂での活躍の説明'),
        }
        ...
        a.update(house_display=houses[fid]['display_name'] if fid else '不明', status='scenario', reason=note,
                 ...
                 availability='not_born' if reserved else 'uncertain' if uncertain else 'available',
                 can_serve_at_start=bool(by_name[name]['start_present']) and not reserved and not uncertain and a['role'] != '元服前')
        if check:
    ...
    report = dict(schema_version=1, batch=BATCH, year=1546, officers=rows,
                  policy='保存済み人物紹介と既存調査の出典付き要約を参照し、一部は自治体・寺院資料を再確認。出自・本拠・後年活動圏を用いた推定配置であり、全件の一次史料による確定ではない。要約参照の15人は原文保存版と区別して記録。',
                  selection_exclusions=baseline['selection_exclusions'],
                  stats=dict(total=100, placed=100, future_reserved=sum(bool(r['after'].get('future_placement_reserved')) for r in rows), with_house=sum(bool(r['after']['house_id']) for r in rows),
    ...
    write(MASTER / 'affiliation_batch10_research.json', report)
...
def render_report(report):
    ...
    for i, row in enumerate(report['officers'], 1):
        ...
        rows.append(f'<tr id="{row["id"]}"><td>{i}</td><td>{e(row["name"])}</td><td>{e(a["house_display"])}</td><td>{e(a["role"])}</td><td>{e(a["district_display"])}</td><td>{e(a["placement_reason"])}{family}<p class="muted">出仕：{"可" if a["can_serve_at_start"] else "無効／保留"} ／ 年代：{"未誕生・将来用仮配置" if a.get("future_placement_reserved") else "未確定" if a["availability"] == "uncertain" else "配置対象"}</p></td><td>{refs}</td></tr>')
    stats = report['stats']
    page = '''<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>所属・立場・配置郡の追加100人 第10組 · 1546年</title><style>
:root{color-scheme:dark;font-family:system-ui,"Yu Gothic UI",sans-serif;background:#10191d;color:#e6ece8}body{margin:24px}h1{font-size:26px}p{line-height:1.7;max-width:1250px}a{color:#a5d1e3}input{padding:10px;width:min(90%,450px);background:#23353b;color:white;border:1px solid #60767b}table{border-collapse:collapse;width:100%;font-size:14px}th,td{padding:12px;border-bottom:1px solid #344449;vertical-align:top;text-align:left}th{background:#25373d;position:sticky;top:0}td:nth-child(2),td:nth-child(3),td:nth-child(4){white-space:nowrap}td:nth-child(6){min-width:320px}td:nth-child(7){min-width:220px}.tablewrap{overflow:auto}.muted{color:#a5b5b4;font-size:12px}.note{border-left:3px solid #d4b574;padding:12px;background:#25312d}
    ...
    page += '<p>' + e(report['policy']) + ' 具体的な地縁が弱い人物は代替地域と明記し、年代・同定の保留を解消した扱いにはしません。未誕生の仮配置は開始時配置と分けて保存します。現在の地図に配置できる人物を選び、既存1498人の設定と能力・家系・父母子情報を保持しています。</p>'
    page += '<p><label>人物・家・郡・根拠を検索 <input id="search" type="search" placeholder="武田、元服前、父 など"></label> <span id="count">100人</span></p><div class="tablewrap"><table><thead><tr><th>No.</th><th>人物</th><th>所属家</th><th>立場</th><th>配置郡</th><th>判断・留保</th><th>資料・参照箇所</th></tr></thead><tbody>' + ''.join(rows) + '</tbody></table></div>'
    ...
    (ROOT / 'docs/officers/affiliation_research_tenth_100.html').write_text(page, encoding='utf-8')
