"""Third 100-person placement overlay, after the two existing research batches."""
import collections
import copy
import hashlib
import html
...
def apply(data, houses, districts, by_name):
    baseline = read(MASTER / 'affiliation_batch3_baseline.json')
    selected = read(HERE / 'affiliation_batch3_selection.json')
    ids = {s['id'] for s in selected}
    assert len(ids) == len(selected) == 100 and ids == set(baseline['selected_ids'])
    profiles = data['officers']
    assert profiles == baseline['officers'], 'The preceding scenario changed; review before regenerating batch 3'
    before = copy.deepcopy(profiles)
    decisions = {}
    for line in (HERE / 'affiliation_batch3_decisions.txt').read_text(encoding='utf-8').splitlines():
    ...
    for fid, name, reference, district_name in [
        ('matsudaira_goi', '松平家（五井）', '松平元心', '宝飯郡'),
        ('matsudaira_nomi', '松平家（能見）', '松平重吉', '額田郡'),
    ]:
        assert fid not in houses
        qid = by_name[reference]['external_id']
        source = read(ROOT / 'data/sources/officers/affiliations_1546' / (qid + '.json'))
        houses[fid] = dict(id=fid, display_name=name, head_reference=reference,
                          pool=[location('三河国', district_name)], district_names=['三河国・' + district_name],
                          basis='系譜と本拠地による家中単位の暫定設定',
                          note='同姓の安祥松平家と家中を区別。本拠圏1郡はゲーム配置候補であり郡全域の史実支配を認定しない。',
                          source_urls=[source['url']], status='scenario_provisional', scenario_batch=BATCH)
    ...
    for s in selected:
        ...
        if name == '小梁川盛宗 (戦国時代)':
            sources[0]['locator'] = '「戰國時代的盛宗」節のみ。室町期の同名人物は除外。'
        if name == '中山勝時':
            sources.append(dict(title='半田市報1556号', url='https://www.city.handa.lg.jp/_res/projects/default_project/_page_/001/003/881/1556.pdf',
                                locator='「ごんぎつね」の中山さまと岩滑領主の説明（地縁の補足）', accessed='2026-09-13', source_kind='municipal_publication'))
        note = decision['note']
        ...
        district = by_key[key]
        uncertain = by_name[name]['temporal_status'] != 'alive' or name in {'ヤジロウ', '松波義親', '水野義純', '内藤源左衛門', '国分盛廉'}
        ...
        reviewed_roles = {'松平元心': '五井松平家当主', '松平重吉': '能見松平家当主',
                          '池田長正': '池田信正の子・1548年家督相続前', '渡辺長': '元服前の毛利家臣・渡辺家当主',
                          '千秋季忠': '熱田大宮司・千秋家の幼少当主', '冷泉為純': '公家（1538年叙爵）'}
        if name in reviewed_roles:
            a['previous_historical_role'] = a['historical_role']
            a['historical_role'] = reviewed_roles[name]
    ...
    write(MASTER / 'affiliation_batch3_research.json', report)
...
<title>所属・立場・配置郡の追加100人 第3組 · 1546年</title><style>
:root{color-scheme:dark;font-family:system-ui,"Yu Gothic UI",sans-serif;background:#10191d;color:#e6ece8}body{margin:24px}h1{font-size:26px}p{line-height:1.7;max-width:1250px}a{color:#a5d1e3}input{padding:10px;width:min(90%,450px);background:#23353b;color:white;border:1px solid #60767b}table{border-collapse:collapse;width:100%;font-size:14px}th,td{padding:12px;border-bottom:1px solid #344449;vertical-align:top;text-align:left}th{background:#25373d;position:sticky;top:0}td:nth-child(2),td:nth-child(3),td:nth-child(4){white-space:nowrap}td:nth-child(6){min-width:320px}td:nth-child(7){min-width:220px}.tablewrap{overflow:auto}.muted{color:#a5b5b4;font-size:12px}.note{border-left:3px solid #d4b574;padding:12px;background:#25312d}
</style><h1>所属・立場・配置郡の追加100人 · 第3組</h1><p><a href="officers_1546.html">全武将一覧</a> ／ <a href="affiliation_research_100.html">第1組</a> ／ <a href="affiliation_research_next_100.html">第2組</a></p>'''
    ...
    page += '<p><label>人物・家・郡・根拠を検索 <input id="search" type="search" placeholder="池田、元服前、父 など"></label> <span id="count">100人</span></p><div class="tablewrap"><table><thead><tr><th>No.</th><th>人物</th><th>所属家</th><th>立場</th><th>配置郡</th><th>判断・留保</th><th>資料・参照箇所</th></tr></thead><tbody>' + ''.join(rows) + '</tbody></table></div>'
    ...
    (ROOT / 'docs/officers/affiliation_research_third_100.html').write_text(page, encoding='utf-8')
