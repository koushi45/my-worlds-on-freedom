...
def main():
    names=read(ROOT/'data/master/officers/fifteenth_registration_names.json')
    ...
    assessed=read(ROOT/'data/master/officers/assessments.json')
    for i in range(0,len(names),20):
        titles=names[i:i+20]
        suffix='_v4' if i==40 else '_v2' if i==60 else ''
        archive=OUT/f'fifteenth_registration_{i//20+1}{suffix}.json'
        ...
        for q,e in data['entities'].items():
            ...
            assert q not in assessed or assessed[q]['cohort']=='fifteenth_100',(q,e.get('labels'))
            ...
            d['in_scope']=True
            d['reason']='第15組の人物伝で戦国期から江戸初期の武家・軍事政務担当または世子として照合。世子の肩書のみでは能力を加点しない。'
            if q in assessed:
                d['reason']+=' 個別参照事項：'+assessed[q]['evidence']
    ...
    assert len(additions)==len(names)==100
    write(OUT/'candidate_index.json',index);write(OUT/'supplemental_index.json',supplement);write(dp,decisions);write(manifest,m)
    write(ROOT/'data/master/officers/fifteenth_registration.json',dict(officers=additions,missing=[]))
