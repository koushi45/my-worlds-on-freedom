...
def main():
    paths = ['data/master/officers/affiliations_1546.json', 'data/master/officers/house_placement_pools_1546.json',
             'data/master/officers/affiliation_batch8_research.json', 'data/derived/officers/officers_1546.json',
             'docs/officers/officers_1546.html', 'docs/officers/affiliation_research_eighth_100.html']
    ...
    assert 'Ran 97 tests' in unit and 'OK' in unit
...
''' + parser.script + '''
callback({target:{value:'石田三成'}});
if(!mockRows.some(r=>!r.hidden&&r.textContent.includes('石田三成'))||mockRows.every(r=>!r.hidden))throw Error('Search failed');
...
'''
    with tempfile.TemporaryDirectory(prefix='affiliation-batch8-') as temp:
    ...
    report = dict(batch='affiliation_batch8_100', unit_tests=97, all_passed=True, html_rows=100,
    ...
    (ROOT / 'data/master/officers/affiliation_batch8_validation.json').write_text(json.dumps(report, ensure_ascii=False, indent=2)+'\n', encoding='utf-8')
