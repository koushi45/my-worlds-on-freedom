...
def main():
    paths = ['data/master/officers/affiliations_1546.json', 'data/master/officers/house_placement_pools_1546.json',
             'data/master/officers/affiliation_batch9_research.json', 'data/derived/officers/officers_1546.json',
             'docs/officers/officers_1546.html', 'docs/officers/affiliation_research_ninth_100.html']
    ...
    assert 'Ran 104 tests' in unit and 'OK' in unit
...
''' + parser.script + '''
callback({target:{value:'織田信忠'}});
if(!mockRows.some(r=>!r.hidden&&r.textContent.includes('織田信忠'))||mockRows.every(r=>!r.hidden))throw Error('Search failed');
...
'''
    with tempfile.TemporaryDirectory(prefix='affiliation-batch9-') as temp:
    ...
    report = dict(batch='affiliation_batch9_100', unit_tests=104, all_passed=True, html_rows=100,
                  ...
                  unchanged_other_officers=1498, future_reservations_inactive_at_start=80)
    (ROOT / 'data/master/officers/affiliation_batch9_validation.json').write_text(json.dumps(report, ensure_ascii=False, indent=2)+'\n', encoding='utf-8')
