"""Eighth 100-person overlay, including explicitly authorized future reservations."""
import collections
import copy
import hashlib
import html
...
def apply(data, houses, districts, by_name):
    baseline = read(MASTER / 'affiliation_batch8_baseline.json')
    selected = read(HERE / 'affiliation_batch8_selection.json')
    ids = {s['id'] for s in selected}
    assert len(ids) == len(selected) == 100 and ids == set(baseline['selected_ids'])
    profiles = data['officers']
    assert profiles == baseline['officers'], 'The preceding scenario changed; review before regenerating batch 8'
    before = copy.deepcopy(profiles)
    decisions = {}
    for line in (HERE / 'affiliation_batch8_decisions.txt').read_text(encoding='utf-8').splitlines():
    ...
    for s in selected:
        ...
        supplements = {
            '立花宗茂': ('豊後高田市文化財室・立花宗茂', 'https://www.city.bungotakada.oita.jp/site/bunkazai/2174.html', '1567年、吉弘鎮理（高橋紹運）の嫡男として筧城に出生という説明'),
            '石田三成': ('長浜市・地域振興と博物館展示の事業資料', 'https://www.city.nagahama.lg.jp/cmsfiles/contents/0000003/3363/4_shiminkyoudou.pdf', '石田家のルーツ展の1560年坂田郡石田村出生と、父正継・兄正澄の展示説明'),
        }
        ...
        district = by_key[key]
        reserved = before[qid]['availability'] == 'not_born'
    ...
    write(MASTER / 'affiliation_batch8_research.json', report)
...
<title>所属・立場・配置郡の追加100人 第8組 · 1546年</title><style>
:root{color-scheme:dark;font-family:system-ui,"Yu Gothic UI",sans-serif;background:#10191d;color:#e6ece8}body{margin:24px}h1{font-size:26px}p{line-height:1.7;max-width:1250px}a{color:#a5d1e3}input{padding:10px;width:min(90%,450px);background:#23353b;color:white;border:1px solid #60767b}table{border-collapse:collapse;width:100%;font-size:14px}th,td{padding:12px;border-bottom:1px solid #344449;vertical-align:top;text-align:left}th{background:#25373d;position:sticky;top:0}td:nth-child(2),td:nth-child(3),td:nth-child(4){white-space:nowrap}td:nth-child(6){min-width:320px}td:nth-child(7){min-width:220px}.tablewrap{overflow:auto}.muted{color:#a5b5b4;font-size:12px}.note{border-left:3px solid #d4b574;padding:12px;background:#25312d}
    ...
    (ROOT / 'docs/officers/affiliation_research_eighth_100.html').write_text(page, encoding='utf-8')
