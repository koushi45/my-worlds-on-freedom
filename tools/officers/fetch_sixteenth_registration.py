...
def main():
    names=read(ROOT/'data/master/officers/sixteenth_registration_names.json')
    ...
    for i in range(0,len(names),20):
        titles=names[i:i+20]
        suffix='_v2' if i in (20,80) else ''
        archive=OUT/f'sixteenth_registration_{i//20+1}{suffix}.json'
        ...
        for q,e in data['entities'].items():
            ...
            assert q not in assessed or assessed[q]['cohort']=='sixteenth_100',(q,e.get('labels'))
            ...
            d['in_scope']=True
            d['reason']='第16組の人物伝で戦国期から江戸初期の武家・軍事政務担当または世子として照合。世子の肩書のみでは能力を加点しない。'
            if q in assessed:
    ...
    write(ROOT/'data/master/officers/sixteenth_registration.json',dict(officers=additions,missing=[]))
