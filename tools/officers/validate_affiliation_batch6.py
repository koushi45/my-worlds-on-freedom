...
def main():
    paths = ['data/master/officers/affiliations_1546.json', 'data/master/officers/house_placement_pools_1546.json',
             'data/master/officers/affiliation_batch6_research.json', 'data/derived/officers/officers_1546.json',
             'docs/officers/officers_1546.html', 'docs/officers/affiliation_research_sixth_100.html']
    ...
    assert 'Ran 83 tests' in unit and 'OK' in unit
...
''' + parser.script + '''
callback({target:{value:'伊達政宗'}});
if(!mockRows.some(r=>!r.hidden&&r.textContent.includes('伊達政宗'))||mockRows.every(r=>!r.hidden))throw Error('Search failed');
...
'''
    with tempfile.TemporaryDirectory(prefix='affiliation-batch6-') as temp:
    ...
    report = dict(batch='affiliation_batch6_100', unit_tests=83, all_passed=True, html_rows=100,
    ...
    (ROOT / 'data/master/officers/affiliation_batch6_validation.json').write_text(json.dumps(report, ensure_ascii=False, indent=2)+'\n', encoding='utf-8')
