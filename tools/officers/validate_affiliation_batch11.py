...
def main():
    paths = ['data/master/officers/affiliations_1546.json', 'data/master/officers/house_placement_pools_1546.json',
             'data/master/officers/affiliation_batch11_research.json', 'data/derived/officers/officers_1546.json',
             'docs/officers/officers_1546.html', 'docs/officers/affiliation_research_remaining_65.html']
...
''' + parser.script + '''
callback({target:{value:'宗晴康'}});
if(!mockRows.some(r=>!r.hidden&&r.textContent.includes('宗晴康'))||mockRows.every(r=>!r.hidden))throw Error('Search failed');
callback({target:{value:'unlikely-no-match-xyz'}});
if(count.textContent!=='0人')throw Error('No-match failed');
callback({target:{value:''}});
if(count.textContent!=='65人'||mockRows.some(r=>r.hidden))throw Error('Clear failed');
'''
    with tempfile.TemporaryDirectory(prefix='affiliation-batch11-') as temp:
    ...
    report = dict(batch='affiliation_batch11_65', unit_tests=118, all_passed=True, html_rows=65,
                  ...
                  unchanged_other_officers=1533, future_reservations_inactive_at_start=3, reference_placements_inactive_at_start=62)
    (ROOT / 'data/master/officers/affiliation_batch11_validation.json').write_text(json.dumps(report, ensure_ascii=False, indent=2)+'\n', encoding='utf-8')
