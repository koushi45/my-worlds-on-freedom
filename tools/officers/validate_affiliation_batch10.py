...
def main():
    paths = ['data/master/officers/affiliations_1546.json', 'data/master/officers/house_placement_pools_1546.json',
             'data/master/officers/affiliation_batch10_research.json', 'data/derived/officers/officers_1546.json',
             'docs/officers/officers_1546.html', 'docs/officers/affiliation_research_tenth_100.html']
    ...
    assert 'Ran 111 tests' in unit and 'OK' in unit
...
''' + parser.script + '''
callback({target:{value:'酒井忠勝'}});
if(!mockRows.some(r=>!r.hidden&&r.textContent.includes('酒井忠勝'))||mockRows.every(r=>!r.hidden))throw Error('Search failed');
...
'''
    with tempfile.TemporaryDirectory(prefix='affiliation-batch10-') as temp:
    ...
    report = dict(batch='affiliation_batch10_100', unit_tests=111, all_passed=True, html_rows=100,
                  ...
                  unchanged_other_officers=1498, future_reservations_inactive_at_start=34)
    (ROOT / 'data/master/officers/affiliation_batch10_validation.json').write_text(json.dumps(report, ensure_ascii=False, indent=2)+'\n', encoding='utf-8')
