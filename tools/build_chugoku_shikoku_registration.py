...
ROOT=core.ROOT
WORK=ROOT/'data/work/political/chugoku_shikoku_registration'
SPEC=WORK/'spec.json'
...
def create_mesh(spec,coast):
    ...
    dst=np.array([nearest_points(core.p2.game_point(*p[3:5]),coast)[1].coords[0] for p in spec['controls']])
...
def build(name):
    ...
    if name=='chugoku':
        ...
        endpoints=['it','bh'];cut=sorted((coast.project(Point(nodes[n])),n,nodes[n]) for n in endpoints)
        ...
        anchor=Point(mesh.transform([spec['seeds']['nagato']])[0])
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--region',choices=['shikoku','chugoku','all'],default='all');args=parser.parse_args()
    for name in ['shikoku','chugoku'] if args.region=='all' else [args.region]:build(name)
