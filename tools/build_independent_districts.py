...
def build():
    ...
    data=resolve(data)
    from curate_requested_district_layout import curate as curate_requested_layout
    data=curate_requested_layout(data)
    from align_district_coastlines import align_mainlands
    ...
    print(f'{len(coast_registry["references"])} canonical coastline references; {len(coast_report["ambiguous_faces"])} ambiguous faces')
